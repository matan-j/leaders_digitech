...
const styles = StyleSheet.create({
  ...
  colReports: { width: '18%', textAlign: 'center' },
  colLessons: { width: '18%', textAlign: 'center' },
  colPay:     { width: '24%', textAlign: 'left' },
...
const SalaryPdfDocument: React.FC<SalaryPdfDocumentProps> = ({ salaryData, selectedMonth, logoUrl }) => {
  ...
  return (
    <Document title="דוח שכר מדריכים">
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.titleBlock}>
            <Text style={styles.titleText}>דוח שכר מדריכים</Text>
        ...
        <View style={styles.table}>
          <View style={styles.tableHead}>
            ...
            <Text style={[styles.tableHeadCell, styles.colReports]}>דיווחים</Text>
          ...
          {salaryData.map((row, idx) => (
            ...
            >
              ...
              <Text style={[styles.tableCell, styles.colReports]}>{row.report_count}</Text>
              <Text style={[styles.tableCell, styles.colLessons]}>{row.lesson_count}</Text>
              <Text style={[styles.tableCell, styles.colPay]}>{fmtMoney(row.total_pay)}</Text>
            </View>
          ))}
          <View style={styles.tableTotalRow}>
            <Text style={[styles.tableTotalCell, styles.colName]}>סה"כ</Text>
            <Text style={[styles.tableTotalCell, styles.colReports]}>{totalReports}</Text>
